...
// 1. Position Details Block Redesign
...
c = c.replace(posRegex, (match) => {
    // We are matching everything inside the grid before the local requirements.
    // Let's rewrite this part completely by finding the input names.
    return \<div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div className="md:col-span-3 flex flex-col gap-4">
                           ...
                           <div>
                              ...
                              <textarea value={pos.description} onChange={(e) => updatePosition(index, "description", e.target.value)} className="w-full border rounded-xl px-3 py-2.5 text-sm bg-white text-black min-h-[90px] resize-y" placeholder="Write detailed job description..." />
                           </div>
                        </div>
