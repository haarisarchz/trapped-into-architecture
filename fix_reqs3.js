...
c = c.replace(globalReqRegex, (match, p1) => {
    return `{/* QUALIFICATION & SKILLS */} <div className="grid grid-cols-1 md:grid-cols-2 gap-4"> <div> <label className="block mb-1.5 text-sm font-medium"> Qualifications </label>${p1}</div> <div> <label className="block mb-1.5 text-sm font-medium"> Skills Required </label>`;
});
