...
const positionBlockRegex = /\s*\{\/\* POSITION \*\/\}\s*<div>\s*<label className="block mb-2 font-medium">\s*Position <span className="text-red-500 text-xl font-bold">\*<\/span>\s*<\/label>[\s\S]*?<\/datalist>\s*<\/div>/;
c = c.replace(positionBlockRegex, '');

const oldExpRegex = /\s*\{\/\* EXPERIENCE \*\/\}\s*<div className="mt-6">[\s\S]*?<\/label>\s*\);\s*\}\)}\s*<\/div>\s*<\/div>/;
...
c = c.replace(/\{\/\* LOCATION \*\/\}/, positionsJSX + '              {/* LOCATION */}');
